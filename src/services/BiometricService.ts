...
class BiometricServiceClass {
    ...
    public async init() {
        const isSecured = await StorageService.getJSON<boolean>('isBiometricSecured');
    ...
    public async authenticate(promptMessage = 'Unlock Spark ADHD'): Promise<boolean> {
    ...
    private handleAppStateChange = (nextState: AppStateStatus) => {
        ...
        if (nextState === 'background' || nextState === 'inactive') {
            ...
            if (this.backgroundedAt && Date.now() - this.backgroundedAt > this.REAUTH_THRESHOLD_MS) {
