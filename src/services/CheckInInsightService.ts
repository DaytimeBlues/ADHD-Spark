...
function buildContext(entries: CheckInEntry[]): string {
    if (entries.length === 0) return 'No recent check-ins.';
...
const CheckInInsightService = {
    async generateInsight(entries: CheckInEntry[]): Promise<CheckInInsight | null> {
        if (entries.length === 0) return null;

        // Return cached insight if still fresh
        const cached = await StorageService.getJSON<CheckInInsight>(INSIGHT_CACHE_KEY);
