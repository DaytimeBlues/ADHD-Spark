...
class GoogleSyncOrchestratorClass {
  ...
  async syncSortedItemsToGoogle(
    ...
    try {
      ...
      const exportedFingerprints =
        await GoogleSyncStateService.getExportedFingerprints();
      const exportedSet = new Set(exportedFingerprints);

      for (
        let index = 0;
        index < items.length;
        index += MAX_EXPORT_CONCURRENCY
      ) {
        ...
        await Promise.allSettled(
          chunk.map(async (item) => {
            ...
            if (
              ['task', 'reminder', 'event'].includes(normalizedItem.category)
            ) {
      ...
      await GoogleSyncStateService.setExportedFingerprints(
        Array.from(exportedSet),
      );
    } catch (error) {
    ...
    try {
      ...
      for (let i = 0; i < pendingMarks.length; i += MAX_MARK_CONCURRENCY) {
        ...
        const marks = await Promise.all(
          chunk.map((id) =>
            this.apiClient.markTaskCompleted(accessToken, listId, id),
          ),
        );
      ...
      await GoogleSyncStateService.writeSyncState({
        listId,
      ...
      if (retryCount < this.MAX_SYNC_RETRIES) {
        await new Promise((r) =>
          setTimeout(r, this.BASE_RETRY_DELAY_MS * Math.pow(2, retryCount)),
        );
    ...
    this.netInfoUnsubscribe = NetInfo.addEventListener((state) => {
      if (state.isConnected) {
        ...
        this.processOfflineQueue().catch((error) => {
          LoggerService.warn({
            service: 'GoogleSyncOrchestrator',
            operation: 'netInfoQueue',
            message: 'Offline queue processing failed',
            error,
          });
        });
      }
    ...
    NetInfo.fetch().then((state) => {
      if (state.isConnected) {
        this.processOfflineQueue().catch((error) => {
          LoggerService.warn({
            service: 'GoogleSyncOrchestrator',
            operation: 'initialFetchQueue',
            message: 'Initial queue processing failed',
            error,
          });
        });
      }
