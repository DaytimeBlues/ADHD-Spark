...
import { enableCosmicTheme, enableE2ETestMode, seedAlexPersona } from './helpers/seed';
...
test.describe('Bubble Features: Interruption & Vignette Check-ins', () => {
    ...
    test.describe('Check-In Mode (Interruption) in Capture Drawer', () => {
        test('Can open Capture Drawer and log progress through Check-in mode', async ({ page }) => {
            ...
            await checkinInput.fill('Checking emails. I should be working on the report.');
    ...
    test.describe('Vignettes in CheckInScreen', () => {
        test('CheckInScreen displays literary vignettes instead of emojis', async ({ page }) => {
            ...
            const nietzscheQuote = page.getByText('“I am a forest, and a night of dark trees.”', { exact: false });
            await expect(nietzscheQuote).toBeVisible();

            const whitmanQuote = page.getByText('“I contain multitudes.”', { exact: false });
